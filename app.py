...
def limpar_eventos_holerite(df):
    ...
    if 'Evento' not in df.columns or 'Valor' not in df.columns:
        return df.copy()
    ...
    df_limpo = df_limpo[
        ~df_limpo['Evento'].str.contains(
            'Salario Bruto|Salario Líquido|Salario Liquido|Salário Bruto|Salário Líquido',
            case=False,
            na=False
        )
    ]

    # 2. Remove "Outros Descontos" APENAS se o valor for POSITIVO
    mask_lixo = (
        df_limpo['Evento'].str.contains('Outros Descontos', case=False, na=False)
    ) & (df_limpo['Valor'] > 0)
...
if file_detalhe is not None and file_holerite is not None:
    ...
    if dados_carregados[0] is not None:
        ...
        # Base reutilizável para evitar NameError em abas que dependem de proventos_unicos
        df_holerite_limpo_base = limpar_eventos_holerite(df_holerite)
        proventos_unicos = sorted(
            df_holerite_limpo_base[df_holerite_limpo_base['Valor'] > 0]['Evento']
            .dropna()
            .unique()
            .tolist()
        )
        tem_proventos = len(proventos_unicos) > 0
        ...
        with tab_geral:
            ...
            custo_bruto = df_detalhe['Salário Bruto'].sum() if 'Salário Bruto' in df_detalhe.columns else 0
            media_salarial = df_detalhe['Salário Bruto'].mean() if 'Salário Bruto' in df_detalhe.columns else 0
            maior_salario = df_detalhe['Salário Bruto'].max() if 'Salário Bruto' in df_detalhe.columns else 0
        ...
        with tab_sec:
            ...
            if 'Secretaria' in df_detalhe.columns and 'Salário Bruto' in df_detalhe.columns:
        ...
        with tab_simulador:
            ...
            if not tem_proventos:
                st.warning("Não foi possível identificar rubricas de proventos no holerite mensal. O simulador de VA ficará limitado.")
            ...
            with st.container(border=True):
                ...
                with col_sim2:
                    ...
                    if tem_proventos:
                        ...
                        # Identificação inteligente das top 4 faixas de VA praticadas
                        ...
                        if tipo_reajuste_va == "Por Faixas (Valores Diferentes)":
                            ...
                            for i, faixa in enumerate(faixas_va_atuais):
                                with col_f1 if i % 2 == 0 else col_f2:
                                    mapa_faixas_va[faixa] = st.number_input(
                                        f"Atual: R$ {faixa:.2f} ➔ Novo:",
                                        min_value=0.0,
                                        value=float(faixa),
                                        step=10.0,
                                        key=f"faixa_{faixa}"
                        ...
                        st.info("Sem rubricas de proventos detectadas. O bloco de VA ficará em valores zerados.")
                        nome_rubrica_va = ""
                        va_atual_df = df_holerite.iloc[0:0].copy()
                        qtd_servidores_va = 0
                        faixas_va_atuais = []
                        tipo_reajuste_va = "Fixo Único (R$)"

                # FUNÇÃO LOCAL DE CÁLCULO DE VA
            ...
            # CÁLCULOS GLOBAIS
            custo_total_atual = df_detalhe['Salário Bruto'].sum() if 'Salário Bruto' in df_detalhe.columns else 0
            ...
            # 2. Impacto VA
            custo_va_atual = va_atual_df['Valor'].sum() if not va_atual_df.empty else 0
            novo_custo_va = va_atual_df['Valor'].apply(calcular_novo_va).sum() if not va_atual_df.empty else 0
            ...
            texto_va_txt = texto_va_txt.rstrip()
...
"""
            ...
            if selecao_sim:
                ...
                holerite_sim = df_holerite_full[
                    (df_holerite_full['Matrícula'] == mat_servidor_sim) &
                ...
                if holerite_sim.empty:
                    ...
                    eventos_sim = limpar_eventos_holerite(holerite_sim).copy()
                    ...
                    # 2. Aplica VA com a nova regra
                    if nome_rubrica_va:
                        mask_va_ind = eventos_sim['Evento'].str.upper() == nome_rubrica_va.upper()
                    else:
                        mask_va_ind = pd.Series([False] * len(eventos_sim), index=eventos_sim.index)
                    ...
                    if mask_va_ind.any():
                        ...
                        if valor_injetar_ind > 0:
                            nova_rubrica = pd.DataFrame([{
                                ...
                                'Evento': nome_rubrica_va if nome_rubrica_va else 'AUXILIO ALIMENTACAO',
                                'Valor': 0.0,
                                'Valor Simulado': valor_injetar_ind
                            }])
        ...
        with tab_auxilio:
            ...
            if not tem_proventos:
                st.warning("Não há rubricas de proventos disponíveis para a análise de auxílio no conjunto carregado.")
            else:
